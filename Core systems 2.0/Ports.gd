...
class Port:
	...
	func _init(label: String, type: Variant.Type) -> void: _label = label; _type = type
...
class InternalWire:
	...
	func _init(port: int, ui_address: UIAddress) -> void: _port = port; _ui_address = ui_address
...
class FinishSignal:
	...
	func _init(object_reference: Node, signal_name: StringName) -> void: _object_reference = object_reference; _signal_name = signal_name
